...
export default function MisCitasPage() {
  ...
  const citasProximas = citas.filter(
    (c) => c.estado === "en_curso" || c.estado === "confirmada" || c.estado === "pendiente" || c.estado === "recordatorio_enviado"
  );
      ...
      <div className="space-y-4">
        {citasMostrar.length === 0 ? (
          ...
          citasMostrar.map((cita) => {
              const esEnCurso = cita.estado === "en_curso";
              return (
                <div
                  key={cita.id}
                  className={`
                    group p-5 rounded-3xl bg-[var(--card)] border transition-all relative overflow-hidden
                    ${esEnCurso
                      ? "border-amber-500/40 shadow-lg shadow-amber-500/10 bg-gradient-to-br from-amber-500/5 to-transparent"
                      : "border-[rgba(201,168,76,0.12)] hover:border-[rgba(201,168,76,0.25)]"
                    }
                  `}
                >
                  {esEnCurso && (
                    <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-amber-500/10 to-transparent rounded-bl-full opacity-60" />
                  )}
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative">
                    <div className="flex items-center gap-4">
                      {esEnCurso && (
                        <div className="absolute -top-1 -left-1 px-2 py-0.5 rounded-full bg-amber-500 text-[var(--dark)] text-[10px] font-black uppercase tracking-widest shadow-lg shadow-amber-500/30 animate-pulse">
                          En Curso
                        </div>
                      )}
